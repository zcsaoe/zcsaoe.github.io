...
# default config
KEYFILE=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/log
[ ! -d "$KEYFILE" ] && mkdir -p "$KEYFILE"
LOGFILE=${KEYFILE}/ddns.txt
if [[ $# -gt 0 ]]; then
    CONFIG_FILE=$1
else
    CONFIG_FILE=${KEYFILE}/../config.json
fi

# API key, see https://www.cloudflare.com/a/account/my-account,
# incorrect api-key results in E_UNAUTH error
CFKEY=$(jq -r '.CFKEY' $CONFIG_FILE)

# Username, eg: user@example.com
CFUSER=$(jq -r '.CFUSER' $CONFIG_FILE)
...
check_ip()
{
    ...
    WAN_IP=`curl -s cip.cc | grep IP | awk -F ': ' '{print $2}'`
    WAN_IP_FILE=$KEYFILE/cf-wan_ip_$CFRECORD_NAME.txt
    ...
    ID_FILE=$KEYFILE/cf-id_$CFRECORD_NAME.txt
    ...
    && [ "$(sed -n '4,1p' "$ID_FILE")" == "$CFRECORD_NAME" ]; then
        ...
        CFZONE_ID=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones?name=$CFZONE_NAME" -H "X-Auth-Email: $CFUSER" -H "X-Auth-Key: $CFKEY" -H "Content-Type: application/json" | jq -r '.result[0].id' ) # grep -Po '(?<="id":")[^"]*' | head -1 
    ...
    if [ "$RESPONSE" != "${RESPONSE%success*}" ] && [ "$(echo $RESPONSE | grep "\"success\":true")" != "" ]; then
        ...
        echo "$(date "+%Y-%m-%d %H:%M:%S") Updated succesfuly! : $WAN_IP" >> $LOGFILE
        # exit
    else
...
for domain in $(jq -r '.CFDOMAIN[]' $CONFIG_FILE); do
    CFRECORD_NAME="$domain"
    CFZONE_NAME=$(echo "$domain" | awk -F. '{print $(NF-1)"."$NF}')
    check_ip
done
